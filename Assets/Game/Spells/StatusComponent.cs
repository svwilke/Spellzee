...
public class StatusComponent : SpellComponent {
	...
	public StatusComponent(string shortDescription, Func<Status> statusFactory) : base() {
	...
	public override void Execute(Spell spell, RollContext context) {
		...
		GetTargets().ForEach(pawn => {
			StatusComponent singleTarget = new StatusComponent(shortDescription, statusFactory);
	...
	public override string GetDescription(Spell spell, RollContext context) {
		...
		string desc = "{0}";
		desc = DescriptionHelper.GetDescriptionPrefix(GetTargetType(), GetTargetGroup()) + desc;
