namespace RetroBlitDemoSuperFlagRun
{
    ...
    public class PlatformPhysics
    {
        ...
        public void ApplyVelocity()
        {
            ...
            float incr = RB.SpriteSize().width / 4.0f;
            Vector3 newPos;
        ...
        private void ResolveCollisions(Vector2 newPos, int axis)
        {
            ...
            int leftTile = (int)Mathf.Floor((int)rect.min.x / RB.SpriteSize().width) - 1;
            int rightTile = (int)Mathf.Ceil((int)rect.max.x / RB.SpriteSize().width);
            int topTile = (int)Mathf.Ceil((int)rect.min.y / RB.SpriteSize().height) - 1;
            int bottomTile = (int)Mathf.Floor((int)rect.max.y / RB.SpriteSize().height);
            ...
            for (int y = topTile; y <= bottomTile; ++y)
            {
                for (int x = leftTile; x <= rightTile; ++x)
                {
                    ...
                    if (tile != RB.SPRITE_EMPTY)
                    {
                        ...
                        Rect2i tileRect = new Rect2i(x * RB.SpriteSize().width, y * RB.SpriteSize().height, RB.SpriteSize().width, RB.SpriteSize().height);
