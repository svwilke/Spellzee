namespace RetroBlitDemoReel
{
    ...
    public class SceneInfiniteMap : SceneDemo
    {
        ...
        private void DrawTilemap(int x, int y)
        {
            ...
            var chunkPixelSize = new Vector2i(RB.MapChunkSize.width * RB.SpriteSize(0).width, RB.MapChunkSize.height * RB.SpriteSize(0).height);
            ...
            if (newTopLeftChunk != mTopLeftChunk)
            {
                ...
                for (int cy = 0; cy <= (mClipRect.height / chunkPixelSize.height) + 1; cy++)
                {
                    for (int cx = 0; cx <= (mClipRect.width / chunkPixelSize.width) + 1; cx++)
                    {
                        ...
                        {
                            RB.MapLoadTMXLayerChunk(mMap, "Terrain", 0, mapPos, chunkPos);
                        }
                    }
                }
        ...
        private void DrawCode(int x, int y)
        {
            ...
            mFormatStr.Append("            @MRB@N.MapLoadTMXLayerChunk(mMap, @S\"Terrain\"@N, @L0@N, mapPos, chunkPos);\n");
            mFormatStr.Append("        }\n");
            mFormatStr.Append("    }\n");
            mFormatStr.Append("\n");
