namespace RetroBlitDemoRetroDungeoneer
{
    ...
    public class EffectPortal : Effect
    {
        ...
        public override void Render()
        {
            ...
            var portalClipRect = new Rect2i(mPortal.e.pos.x * RB.SpriteSize(0).width, mPortal.e.pos.y * RB.SpriteSize(0).height, RB.SpriteSize(0).width, RB.SpriteSize(0).height);
            ...
            var portalPos = new Vector2i(mPortal.e.pos.x * RB.SpriteSize(0).width, (mPortal.e.pos.y + (1.0f - rise)) * RB.SpriteSize(0).height);
            var portalCenter = new Vector2i(portalPos.x + (RB.SpriteSize(0).width / 2), (mPortal.e.pos.y * RB.SpriteSize(0).height) + RB.SpriteSize(0).height - 2);
            ...
            var outerLightSize = new Vector2i((RB.SpriteSize(0).width * 2 * 0.8f) - mShrinkLight, (RB.SpriteSize(0).height * 0.8f) - mShrinkLight);
            var innerLightSize = new Vector2i((RB.SpriteSize(0).width * 2 * 0.5f) - mShrinkLight, (RB.SpriteSize(0).height * 0.5f) - mShrinkLight);
