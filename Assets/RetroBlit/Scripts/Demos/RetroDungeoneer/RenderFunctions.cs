        ...
        private static void RenderMapBorder()
        {
            ...
            RB.DrawRectFill(
                new Rect2i(
                    ...
                    0,
                    RB.DisplaySize.width,
                    game.map.size.height * RB.SpriteSize(0).height),
                    game.map.unexploredColor);

            RB.DrawRectFill(
                new Rect2i(
                    game.map.size.width * RB.SpriteSize(0).width,
                    0,
                    RB.DisplaySize.width,
                    game.map.size.height * RB.SpriteSize(0).height),
                    game.map.unexploredColor);
            ...
            RB.DrawRectFill(
                new Rect2i(
                    ...
                    (game.map.size.width * RB.SpriteSize(0).width) + (RB.DisplaySize.width * 2),
                    RB.DisplaySize.height),
                    game.map.unexploredColor);
        ...
        private static void RenderTargeting()
        {
            ...
            if (tilePos.x != -1)
            {
                var renderPos = new Vector2i(
                    tilePos.x * RB.SpriteSize(0).width,
                    tilePos.y * RB.SpriteSize(0).height);
