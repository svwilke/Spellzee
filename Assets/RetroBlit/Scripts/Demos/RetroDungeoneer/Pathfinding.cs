namespace RetroBlitDemoRetroDungeoneer
{
    ...
    public class FloodMap
    {
        ...
        {
            int w = RB.SpriteSize(0).width;
            int h = RB.SpriteSize(0).height;
            int i = 0;
            ...
            for (int y = 0; y < mSize.height; y++)
            {
                for (int x = 0; x < mSize.width; x++)
                {
                    ...
                    {
                        RB.Print(new Rect2i(x * w, y * h, RB.SpriteSize(0).width, RB.SpriteSize(0).height), Color.red, RB.ALIGN_H_CENTER | RB.ALIGN_V_CENTER, C.FSTR.Set(cost / 10));
                    }
