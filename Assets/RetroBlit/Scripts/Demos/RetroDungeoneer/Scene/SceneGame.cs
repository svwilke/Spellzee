namespace RetroBlitDemoRetroDungeoneer
{
    ...
    public class SceneGame : Scene
    {
        ...
        public static Vector2i GetMouseTilePos()
        {
            ...
            mousePos.x -= (RB.DisplaySize.width / 2) - (RB.SpriteSize(0).width / 2);
            mousePos.y -= (RB.DisplaySize.height / 2) - (RB.SpriteSize(0).height / 2);

            var tilePos = new Vector2i(mousePos.x / RB.SpriteSize(0).width, mousePos.y / RB.SpriteSize(0).height);
        ...
        private void HandleResult(ResultSet resultSet, Result result, out bool tookTurn)
        {
            ...
            switch (result.type)
            {
                        ...
                        if ((delta.x != 0 || delta.y != 0) && !mGameMap.IsBlocked(mPlayer.e.pos + delta))
                        {
                            ...
                            else
                            {
                                ...
                                if (!stuckInWeb)
                                {
                                    ...
                                    RB.SoundPlay(C.SOUND_FOOT_STEP, 0.66f, RandomUtils.RandomPitch(0.1f));
                                ...
                                else
                                {
                                    ...
                                    RB.SoundPlay(C.SOUND_WEB, 0.5f, RandomUtils.RandomPitch(0.1f));
                                }
                            }
                ...
                case Result.Type.ItemAdded:
                    ...
                    RB.SoundPlay(C.SOUND_INVENTORY, 1, RandomUtils.RandomPitch(0.1f));
