namespace RetroBlitDemoRetroDungeoneer
{
    ...
    public class Console
    {
        ...
        public void Render()
        {
            ...
            {
                var lineSize = RB.PrintMeasure(C.FONT_SMALL, new Rect2i(0, 0, size.width, 9999), RB.TEXT_OVERFLOW_WRAP, curLine.Value);

                // Draw text
                RB.AlphaSet(alpha);
                RB.Print(C.FONT_SMALL, new Rect2i(4, RB.DisplaySize.height - yOffset - 4 - lineSize.height, size.width, 9999), Color.white, RB.TEXT_OVERFLOW_WRAP, curLine.Value);
                RB.AlphaSet(255);
                alpha -= 15;
                if (alpha < 15)
                {
